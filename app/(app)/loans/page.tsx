...
export default function LoansPage() {
  ...
  const { given, received, loading, error, markPaid, deleteLoan, refresh } =
    useLoans();
    ...
    <main className="min-h-screen px-5 pb-safe pt-safe">
      ...
      <div className="mb-4 flex border-b border-border">
        <button
          ...
          className={`flex-1 pb-2.5 text-center text-[15px] font-medium transition-colors ${
            tab === "given"
            ...
            {list.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 pt-20">
                ...
                <p className="text-[15px] font-medium text-foreground">
                  {tab === "given"
                    ? "No has prestado dinero"
                    : "No te han prestado dinero"}
                </p>
                <p className="text-sm text-muted-foreground">
                  {tab === "given"
                    ? "Registra préstamos que hagas a otros"
                    : "Registra préstamos que te hayan hecho"}
                </p>
                <Link
                  href={`/loans/new?type=${tab}`}
                ...
                {list.map((loan, i) => (
                  ...
                  >
                    <LoanCard
                      ...
                      onEdit={(id) =>
                        router.push(`/loans/${id}/edit?type=${tab}`)
                      }
